...
const Input = forwardRef<HTMLInputElement, InputProps>(
	({ type, name, icon, prefix, className, ...props }, ref) => {
		...
		return (
			<div
				className={cn(
					'border-border has-focus:bg-muted relative flex h-10 w-full items-center rounded-md border p-0 text-white has-focus-visible:outline-2 has-focus-visible:outline-offset-2',
					className
				)}
			>
				{prefix ? (
